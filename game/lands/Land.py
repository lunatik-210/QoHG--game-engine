...
class Land:
    def __init__(self, items, bioms, map_generator):
        self.items = items
        self.bioms = bioms
        ...
        self.humidity = Humidity(self.lsize, self.bioms['humidity'])
    ...
    def init_player(self):
        ...
        while not found:
            ...
            if self.value(pos) == self.items['default'] and self.bioms['prairie'] == self.humidity.value(pos.x, pos.y):
                found = True
    ...
    def add_path_to_player(self, destination):
        path = get_path(self.player.pos, destination, self.get_land(), self.items['walkable_objects'])
        if path is None:
            return
        path.reverse()
    ...
    def value(self, pos):
        ...
        monsters = self.bioms[biom]['monsters']

        # choose monster
        if (monsters is not None) and (self.land[pos.x][pos.y] in self.items['walkable_objects']):
    ...
    def update(self, p1, p2):
        ...
        monsters = self.items['monsters']
        for x in range(p1.x, p2.x):
            for y in range(p1.y, p2.y):
                for monster in monsters:
                    if self.land[x][y] == monsters[monster]:
                        self.land[x][y] = self.bioms[self.humidity.value(x,y)]['default']
                        count = 0
                        while count < 5:
                            new_x = int(random.uniform(-2, 2)) + x
                            new_y = int(random.uniform(-2, 2)) + y
                            if self.land[new_x][new_y] in self.items['walkable_objects']:
    ...
    def get_block_id(self, val, biom):
        for block in self.bioms[biom]['objects']:
            if block[0][0] <= val <= block[0][1]:
                return block[1]
        return self.bioms[biom]['default']
